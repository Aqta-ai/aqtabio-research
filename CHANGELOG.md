...
`/auth/evaluator-token` and `/auth/evaluator-exchange`.
  ...
  Aligns the route names with the dashboard's preview-access cookie
  naming and the README's "evaluator" terminology. Request body field
  on the exchange endpoint renamed `judge_token` → `evaluator_token`.
